...
import type {
  ApiKey, AuditRow, Entitlements, IssuedApiKey, PlatformUser, SystemStatus,
  TenantOverview,
} from "@/lib/types";
import {
  Button, DataTable, KeyValue, Pager, QuotaBar, Select, Sparkline, StatusPill,
  TextInput, formatBytes, formatWhen,
...
import FinancialsView from "@/components/admin/financials-view";
import MarketOpsView from "@/components/admin/market-ops-view";
...
export default function AdminConsole({ initialTab = "overview" }: { initialTab?: string }) {
  ...
  return (
    ...
    <div className="mx-auto max-w-[1500px] space-y-4 p-4 lg:p-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold text-[var(--text)]">Enterprise Admin Panel</h1>
          <p className="mt-0.5 text-xs text-[var(--text-muted)]">
            The central control room for EquityPilot AI.
          </p>
        </div>
      </header>
      ...
      <div className="flex gap-4">
        ...
        <div className="min-w-0 flex-1 space-y-4">
          <AdminMobileNav active={tab} onChange={setTab} />
          ...
          {tab === "recycle-bin" && <RecycleBinView />}
          {tab === "companies" && <CompaniesView />}
          {tab === "financials" && <FinancialsView />}
          {tab === "market" && <MarketOpsView />}
          {tab === "ai-score" && <AIOpsView />}
...
function SystemStatusRow() {
  const { data, isLoading } = useQuery<SystemStatus>({
    queryKey: ["admin", "system-status"], queryFn: adminApi.systemStatus,
  });

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-[var(--text)]">System status</h2>
        {data && (
          <span className="text-[0.625rem] text-[var(--text-muted)]">
            Market {data.market_open} · checked {formatWhen(data.generated_at)}
          </span>
        )}
      </div>
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
          {(data?.components ?? []).map((c) => (
            <Card key={c.name}>
              <CardBody className="space-y-1">
                ...
                {c.detail && (
                  <div className="truncate text-[0.625rem] text-[var(--text-muted)]" title={c.detail}>
                    {c.detail}
                  </div>
                )}
              </CardBody>
          ...
          <Card>
            <CardBody>
              <Stat label="Companies" value={(data?.companies ?? 0).toLocaleString("en-IN")} />
            </CardBody>
          </Card>
          <Card>
            <CardBody>
              <Stat label="Users" value={(data?.users ?? 0).toLocaleString("en-IN")} />
            </CardBody>
          </Card>
        </div>
      )}
