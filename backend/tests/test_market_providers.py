...
class TestNSEProvider:
    ...
    def test_fetch_quote_is_the_narrow_entry_point(self):
        """Live paths must call fetch_quote(), not a non-existent get_quote()."""
        assert NSEIndiaProvider.fetch_quote is not BaseMarketProvider.fetch_quote
        assert not hasattr(NSEIndiaProvider, "get_quote")


class TestNSEHttp403Circuit:
    """When NSE/Akamai returns HTTP 403, fail fast and stay dark for a cooldown.

    Confirmed from AWS EC2: the quote endpoint answers 403. Retrying it
    (session prime + quote, 8–15s each) is what made RELIANCE take ~22s
    on every request. A 403 is provider-unavailable, not a per-symbol miss.
    """

    @staticmethod
    def _http_error(code: int, body: bytes = b"Access Denied"):
        import io
        import urllib.error

        return urllib.error.HTTPError(
            "https://www.nseindia.com/api/quote-equity?symbol=RELIANCE",
            code, "Forbidden", {}, io.BytesIO(body),
        )

    @staticmethod
    def _blocked_opener(error, calls):
        class _Opener:
    ...
    def test_http_403_is_provider_unavailable_not_retried(self):
        provider = NSEIndiaProvider()
        calls = {"n": 0, "urls": []}
        provider._opener = self._blocked_opener(self._http_error(403), calls)

        with pytest.raises(ProviderError) as caught:
            provider.fetch("RELIANCE")
        assert "403" in str(caught.value)
        assert "unavailable" in str(caught.value).lower()
        # One quote call, no second attempt — 403 is not transient.
        assert calls["n"] == 1
        assert not provider.available

    def test_http_403_on_session_prime_trips_the_circuit(self):
        provider = NSEIndiaProvider()
        calls = {"n": 0, "urls": []}
        # Force a fresh session so priming runs.
        provider._opener = None

        def fake_build_opener(*_a, **_k):
            return self._blocked_opener(self._http_error(403), calls)
        ...
        assert "403" in str(caught.value)
        assert not provider.available
        # Priming 403 must not be followed by a quote attempt.
        assert calls["n"] == 1

    def test_circuit_skips_nse_on_the_next_call(self):
        provider = NSEIndiaProvider()
        calls = {"n": 0, "urls": []}
        provider._opener = self._blocked_opener(self._http_error(403), calls)
        ...
        with pytest.raises(ProviderRateLimited):
            provider.fetch_quote("TCS")
        elapsed_ms = (__import__("time").perf_counter() - started) * 1000

        assert calls["n"] == first          # no new HTTP
        assert elapsed_ms < 250             # fail-fast, not another 8–20s wait

    def test_router_falls_through_and_reports_price_none(self):
        nse = NSEIndiaProvider()
        nse._opener = self._blocked_opener(self._http_error(403), {"n": 0, "urls": []})

        router = MarketDataRouter(providers=[nse], ttl_cache=TTLCache())
        result = router.fetch("RELIANCE", use_cache=False)
        assert result.source == SOURCE_NONE
        assert result.snapshot.quote.price is None
    ...
    def test_router_skips_nse_and_yahoo_after_403(self):
        ...
        class CountingNSE(NSEIndiaProvider):
            def fetch(self, ticker, **kwargs):
                nse_calls["n"] += 1
                # Simulate the real 403 path: trip the cooldown, then raise.
    ...
    def test_cooldown_expires_and_nse_is_tried_again(self):
        provider = NSEIndiaProvider()
        provider.mark_unavailable(0.01)
        assert not provider.available
        __import__("time").sleep(0.03)
        assert provider.available

    def test_reset_circuit_clears_the_403_cooldown(self):
        provider = NSEIndiaProvider()
